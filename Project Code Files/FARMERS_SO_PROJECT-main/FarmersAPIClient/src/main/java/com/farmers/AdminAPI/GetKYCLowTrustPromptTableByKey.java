...
public class GetKYCLowTrustPromptTableByKey {
	
//private static final Logger logger = LogManager.getLogger(Constants.LOGGER_NAME);
	
	public JSONObject start(String url, String callid, int tenantid, String key, String businessunit, int conntimeout, int readtimeout, LoggerContext context) {
		
		Logger logger = context.getLogger(Constants.ADMIN_LOGGER_NAME);
		logger.info(callid+" : Params passed = "+url+" ,"+tenantid+", "+key+", "+businessunit+", "+conntimeout+", "+readtimeout);
		JSONObject resp = new JSONObject();
		JSONObject reqbody = new JSONObject();
		reqbody.put("tenantid", tenantid);
		reqbody.put("key", key);
		reqbody.put("callerid", callid);
		reqbody.put("businessunit", businessunit);
	...
	/*  public static void main(String args[]) {
	  ...
	  String url = "https://104.156.46.196/IVRAPI/api/KYCLowTrustPrompt/GetKYCLowTrustPromptTableByKey";
	  int tenantid = 2; 
	  String key = "112";
	  String callid = "test123";
	  String businessunit = "21st Century";
	  
	  GetKYCLowTrustPromptTableByKey test = new GetKYCLowTrustPromptTableByKey();
	  JSONObject response = test.start(url, callid, tenantid, key, businessunit, 10000, 10000, context);
