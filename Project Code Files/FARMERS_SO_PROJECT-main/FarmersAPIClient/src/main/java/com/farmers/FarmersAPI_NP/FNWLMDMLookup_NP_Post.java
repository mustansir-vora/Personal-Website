...
public class FNWLMDMLookup_NP_Post {
	
	public JSONObject start(String url, String tid, String ani, String policyNumber, String dob, String zip, String ssn, String lookupType, String region, int conntimeout, int readtimeout, LoggerContext context) {
		Logger logger = context.getLogger(Constants.FARMERS_LOGGER_NAME);
		logger.info(tid+" : Parameters Passed from CVP : url :: " + url + ", tid :: " + tid + ", lookupType :: " + lookupType + ", ani :: " + ani + ", policyNumber :: "+ policyNumber + ", dob :: " + dob + ", zip :: " + zip + ", ssn :: " + ssn + ", connTimeOut :: " + conntimeout +", readTimeOut :: " + readtimeout);
		...
		try {
			...
			String requestBody = formRequestBody(tid, lookupType, ani, policyNumber, dob, zip, ssn, logger);
	...
	public String formRequestBody(String tid, String lookupType, String ani, String policyNunmber, String dob, String zip, String ssn, Logger logger) {
		...
		try {
			...
			criteria.put("businessUnit", "FNWL-IVR");
			
			if (lookupType.equalsIgnoreCase("DOB")) {
				criteria.put("dateOfBirth", dob);
				criteria.put("zip", zip);
				criteria.put("ssnLast4", ssn);
			}
			else if (lookupType.equalsIgnoreCase("POLICY")) {
				criteria.put("PCN", policyNunmber);
			}
			else {
				criteria.put("phoneNumber", ani);
			}
