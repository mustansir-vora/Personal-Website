...
public class FWSBillingLookup_NP_Get {
	
//private Logger logger = LogManager.getLogger(Constants.LOGGER_NAME);
	
	public  JSONObject start(String url, String tid, String policysource, String policystate, String lob, int conntimeout, int readtimeout, LoggerContext context, String region) {

		Logger logger = context.getLogger(Constants.FARMERS_LOGGER_NAME);
		logger.info(tid+" : Request parameters passed = "+url+", "+tid+", "+policysource+", "+policystate+", "+lob+", "+conntimeout+", "+readtimeout);
		...
		try {
			...
			HashMap<String, String> headers = (HashMap<String, String>) getHeaders(Scope, tid, policysource, policystate, lob, logger, region);
	...
	private  Map<String, String> getHeaders(String Scope, String tid, String policysource, String policystate, String lob, Logger logger, String region ) {
	...
	public  JSONObject formatErrorResponse(String error, int statusCode) {
	  ...
	  String url = "https://api-ss.farmersinsurance.com/billingms/v1/billingaccounts/007068754/billingSummary?policyNumber=A7994505830&effectiveDate=2023-12-24&suffix=0&companyCode=00";
	  String tid = "Ciscotest"; 
	  String policysource = "Met360"; //Policy Source = Met360 --- Either value bases on previous api response needs to be passed
	  String policystate = "GA"; //dynamic value based on previous API response -FWSPolicyLookup_Post API - same as above 
	  String lob = "A";
	  FWSBillingLookup_NP_Get test = new FWSBillingLookup_NP_Get();
	  JSONObject resp = test.start(url, tid, policysource, policystate, lob, 10000, 10000, context,"uat");
