...
public class GetStateTableByAreaCode {

	public JSONObject start(String url, String callid, int tenantid, String areacode, int conntimeout, int readtimeout, LoggerContext context) {
		Logger logger = context.getLogger(Constants.ADMIN_LOGGER_NAME);
		logger.info(callid + " : Params passed = "+url+" ,"+tenantid+", "+areacode+", "+conntimeout+", "+readtimeout);
		JSONObject resp = new JSONObject();
		JSONObject reqbody = new JSONObject();
		reqbody.put("tenantid", tenantid);
		reqbody.put("callerid", callid);
		reqbody.put("areacode", areacode);
	...
	private Map<String, String> getHeaders(Logger logger, String callid) {
	  ...
	  String url = "https://104.156.46.196/IVRAPI/api/State/GetStateTableByAreaCode"; 
	  int tenantid = 2; 
	  String callid = "test123";
	  String areacode = "209";
	  
	  GetStateTableByAreaCode objCheckAniExistByKey = new GetStateTableByAreaCode();
	  JSONObject response = objCheckAniExistByKey.start(url, callid, tenantid, areacode, 10000, 10000, context);
