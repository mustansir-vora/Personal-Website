...
public class UpdateEmployeeServiceLineDetails {
	
	public JSONObject start(String url, String callid, String loginid, String leaveapplied, int conntimeout, int readtimeout, LoggerContext context) {
		Logger logger = context.getLogger(Constants.ADMIN_LOGGER_NAME);
		logger.info(callid+" : Params passed = "+url+" ,"+loginid+", "+conntimeout+", "+readtimeout);
		JSONObject resp = new JSONObject();
		JSONObject reqbody = new JSONObject();
		JSONObject reportdata = new JSONObject();
		reqbody.put("loginid", loginid);
		reqbody.put("leaveapplied", leaveapplied);
	...
	private Map<String, String> getHeaders(Logger logger, String callid) {
  	...
  	public static void main(String[] args) {
		  ...
		  String url = "https://104.156.46.196/IVRAPI_DEV/api/EmployeeServiceLine/UpdateEmployeeServiceLineDetails"; 
		  String callid = "test123";
		  String loginid = "122100126";
		  String leaveapplied = "1";
		  String iexid = "testID";
		  String leavetype = "testPTO";
		  String fullorpartial = "testFull";
		  String leavestart = "test 04-04-24";
		  String leaveend = "test 05-04-24";
		  
		  UpdateEmployeeServiceLineDetails test = new UpdateEmployeeServiceLineDetails();
		  JSONObject response = test.start(url, callid, loginid, leaveapplied, 10000, 10000, context);
	}  
