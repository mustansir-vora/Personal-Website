...
public class AgentInfo_NP_Post {
	
	//private Logger logger = LogManager.getLogger(Constants.LOGGER_NAME);

	public  JSONObject start(String url, String tid, String businessunit, String phonenumber,String userid, String systemname, int conntimeout, int readtimeout, LoggerContext context, String region) {

		Logger logger = context.getLogger(Constants.FARMERS_LOGGER_NAME);
		logger.info(tid+" : Parameters Passed from CVP : "+url+", "+tid+", "+businessunit+", "+phonenumber+", "+userid+", "+systemname+", "+conntimeout+", "+readtimeout+","+region);
	...
	public  String formRequestBody(String businessunit, String phonenumber,String userid, String systemname) {
	  ...
	  AgentInfo_NP_Post test = new AgentInfo_NP_Post();
	  resp = test.start(url, tid, "IVRPLA", "5074747005","VRS-101", "VRS", 10000, 10000 ,context, region); 
	  }
