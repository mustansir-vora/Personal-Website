...
public class RiskAttritionAPILookup_Post {
	
	public JSONObject start(String url, String tid, String policynumber, String policysource, String timestamp, int conntimeout, int readtimeout, LoggerContext context) {
		...
		try {
			...
			HashMap<String, String> headers = (HashMap<String, String>) getHeaders(Scope, tid,policysource, logger);
	...
	private Map<String, String> getHeaders(String Scope, String tid, String policysource, Logger logger) {
	...
	public JSONObject formatErrorResponse(String error, int statusCode) {
	  ...
	  String timestamp="";
	  RiskAttritionAPILookup_Post test = new RiskAttritionAPILookup_Post();
	  resp = test.start(url, tid, policynumber, policysource, timestamp, 10000, 10000, context);
	  } 
