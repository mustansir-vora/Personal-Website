...
public class PointValidateCardandRoutingNumber_NP_Post {
	
//private Logger logger = LogManager.getLogger(Constants.LOGGER_NAME);
	
	public JSONObject start(String url, String tid, String accountnumberlength, String cardnumber, String routingnumber, String paymentmethodtype, int conntimeout, int readtimeout, LoggerContext context, String region) {
	...
	public JSONObject formatErrorResponse(String error, int statusCode) {
	  ...
	  PointValidateCardandRoutingNumber_NP_Post test = new PointValidateCardandRoutingNumber_NP_Post();
	  resp = test.start(url, tid, accountnumber, cardnumber, routingnumber, paymentmethodtype, 10000, 10000, context,"uat"); //System.out.println("final response = "+resp); }
