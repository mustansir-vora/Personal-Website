...
public class GetRoutingChildObjects {
	
	public JSONObject start(String url, String callid, int tenantid, String key, String routingkey, int conntimeout, int readtimeout, LoggerContext context) {
		Logger logger = context.getLogger(Constants.ADMIN_LOGGER_NAME);
		logger.info(callid + " : Params passed = "+url+" ,"+tenantid+", "+key+", "+routingkey+""+conntimeout+", "+readtimeout);
		JSONObject resp = new JSONObject();
		JSONObject reqbody = new JSONObject();
		reqbody.put("tenantid", tenantid);
		reqbody.put("callerid", callid);
		reqbody.put("key", key);
		reqbody.put("routingkey", routingkey);
	...
	private Map<String, String> getHeaders(Logger logger, String callid) {
	 ...
	 /* public static void main(String args[]) {
      ...
      String url = "https://104.156.47.196/IVRAPI_DEV/api/Routing/GetRoutingChildObjects";
	  ...
	  JSONObject response = objGetRoutingChildObjects.start(url, callid, tenantid, key, routingkey, 10000, 10000, context);
