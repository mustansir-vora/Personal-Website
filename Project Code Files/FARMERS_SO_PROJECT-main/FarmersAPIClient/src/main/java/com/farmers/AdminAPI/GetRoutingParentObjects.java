...
public class GetRoutingParentObjects {
	
	public JSONObject start(String url, String callid, int tenantid, String key, String dnis, String dnisgroup, String category, String lineofbusiness, int conntimeout, int readtimeout, LoggerContext context) {
		Logger logger = context.getLogger(Constants.ADMIN_LOGGER_NAME);
		logger.info(callid + " : Params passed = "+url+" ,"+tenantid+", "+key+", "+dnis+", "+dnisgroup+", "+category+", "+lineofbusiness+", "+conntimeout+", "+readtimeout);
		...
		reqbody.put("lineofbusiness", lineofbusiness);
	...
	/*  public static void main(String args[]) {
      ...
      String url = "https://104.156.47.196/IVRAPI_DEV/api/Routing/GetRoutingParentObjects";
	  //String url = "https://EC2AMAZ-ALDVADR/fiivrapi/api/AniGroup/CheckAniExistByKey"; 
	  int tenantid = 2; 
	  String key = "Anigroup donotdelete";
	  ...
	  GetRoutingParentObjects objGetRoutingParentObjects = new GetRoutingParentObjects();
	  JSONObject response = objGetRoutingParentObjects.start(url, callid, tenantid, key, dnis, dnisgroup, category, lob, 10000, 10000, context);
