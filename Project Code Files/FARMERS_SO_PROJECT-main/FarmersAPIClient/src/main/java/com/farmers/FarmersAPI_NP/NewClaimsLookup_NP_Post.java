...
public class NewClaimsLookup_NP_Post {

	//private static Logger logger = LogManager.getLogger(AccountLinkAniLookup_Post.class.getName());
	
		public JSONObject start(String url, String tid, String telephonenumber,String functionName,String claimNumber,String policyNumber,int conntimeout, int readtimeout, LoggerContext context, String region) {
			
			Logger logger = context.getLogger(Constants.FARMERS_LOGGER_NAME);
			logger.info(tid+" : Parameters Passed from CVP : "+url+", "+tid+", "+telephonenumber+","+functionName+","+claimNumber+","+policyNumber+" "+conntimeout+", "+readtimeout+","+region);
			...
			try {
				...
				String requestBody = formRequestBody(telephonenumber,functionName,claimNumber,policyNumber);
		...
		public  String formRequestBody(String telephonenumber,String functionName,String claimNumber,String policyNumber) {
			...
			if (telephonenumber == null || telephonenumber.isEmpty() || telephonenumber.equalsIgnoreCase("") && 
					functionName == null || functionName.isEmpty() || functionName.equalsIgnoreCase("") ) {
				
				searchCriteria.put("phoneNumber", "default");
				searchCriteria.put("functionName", "claimLiteSearch");
				searchCriteria.put("claimNumber", "default");
				searchCriteria.put("policyNumber", "default");
			}
			else {
				searchCriteria.put("phoneNumber", telephonenumber);
				searchCriteria.put("functionName", functionName);
				searchCriteria.put("claimNumber", claimNumber);
		 ...
		 public static void main(String[] args) {
		  ...
		  String url = "https://api-np-ss.farmersinsurance.com/CCSIT/claimsms/v4/claims?operation=claimSearch&queryLevel=lite";
		  String telephonenumber = "3434343434"; 
		  String tid = "CiscoTest21";
		  String functionName = "claimLiteSearch";
		  String claimNumber = "";
		  String policyNumber = "";
		  JSONObject resp = null;
		  String region="UAT";
		  
		  NewClaimsLookup_NP_Post test = new NewClaimsLookup_NP_Post(); resp = 
		  test.start(url, tid, telephonenumber,functionName,claimNumber,policyNumber, 10000, 10000, context,region);
