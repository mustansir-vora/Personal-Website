...
public class Afni_Post {
	
//private Logger logger = LogManager.getLogger(Constants.LOGGER_NAME);
	
	public  JSONObject start(String url, String tid, String callerani, String policynumber,String accountnumber,String originaldnis,String originaldnisdescription,String zipcode, String quotenumber, String state, int conntimeout, int readtimeout, LoggerContext context) {
		
		Logger logger = context.getLogger(Constants.FARMERS_LOGGER_NAME);
		logger.info(tid+" : Parameters Passed from CVP : "+url+", "+tid+", "+callerani+", "+policynumber+", "+accountnumber+", "+originaldnis+", "+originaldnisdescription+", "+zipcode+", "+quotenumber+", "+state+", "+conntimeout+", "+readtimeout);
		...
		try {
			...
			String requestBody = formRequestBody(callerani, policynumber, tid, accountnumber, originaldnis, originaldnisdescription, zipcode, quotenumber, state);
	...
	public String formRequestBody(String callerani, String policynumber, String ivrcallid, String accountnumber, String originaldnis, String originaldnisdescription, String zipcode, String quotenumber, String state) {
	...
	private boolean isExpired(Date storedTimestamp) {
	  ...
	  resp = test.start(url, tid, "5122333198", "", "", "15541452123", "", "", "", "", 10000, 10000, context); //
