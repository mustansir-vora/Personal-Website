...
public class CheckFWSZipCode {
	
public JSONObject start(String url, String callid, int tenantid, String code, int conntimeout, int readtimeout, LoggerContext context) {
		
		Logger logger = context.getLogger(Constants.ADMIN_LOGGER_NAME);
		logger.info(callid+" : Params passed = "+url+" ,"+tenantid+", "+code+", "+conntimeout+", "+readtimeout);
	...
	/*  public static void main(String args[]) {
	  ...
	  String url = "https://104.156.46.196/IVRAPI/api/FWSZipCode/CheckFWSZipCode"; 
	  int tenantid = 2; 
	  String callid = "test123";
	  String code = "Anigroup donotdelete";
	  
	  GetAniGroupTableByKey test = new GetAniGroupTableByKey();
	  JSONObject response = test.start(url, callid, tenantid, code, 10000, 10000, context);
