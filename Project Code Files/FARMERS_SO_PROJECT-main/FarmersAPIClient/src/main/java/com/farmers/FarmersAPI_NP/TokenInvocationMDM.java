...
public class TokenInvocationMDM {
	...
	public String getAccessToken(String scope, Logger logger, String tid) {
        ...
        storedTokenBean = TokenStorageMDM.getInstance().getToken(scope, logger, tid);
        if(storedTokenBean != null) {
            ...
            if (extoken != null && !isExpired(exDate)) {
        		...
        		TokenStorageMDM.getInstance().putToken(scope, tknBean, logger, tid);
                rtnToken = newToken;
            } 
        } else {
	...
	public String getToken(String Scope, Logger logger, String tid) {
		...
		try {
			...
			String Base64 = prop.getProperty("Farmers.Mdm.Host.Authorization");
			String Client_ID = prop.getProperty("Farmers.Mdm.OAuth.ClientID");
			String Client_Secret = prop.getProperty("Farmers.Mdm.OAuth.ClientSecret");
