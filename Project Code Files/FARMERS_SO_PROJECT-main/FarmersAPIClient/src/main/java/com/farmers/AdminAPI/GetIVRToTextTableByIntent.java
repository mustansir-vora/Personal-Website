...
public class GetIVRToTextTableByIntent {
	
public JSONObject start(String url, String callid, int tenantid, String intent, int conntimeout, int readtimeout, LoggerContext context) {
		
		Logger logger = context.getLogger(Constants.ADMIN_LOGGER_NAME);
		logger.info(callid+" : Params passed = "+url+" ,"+tenantid+", "+intent+", "+conntimeout+", "+readtimeout);
	...
	private Map<String, String> getHeaders(Logger logger, String callid) {
	  ...
	  String url = "https://104.156.46.196/IVRAPI/api/IVRToText/GetIVRToTextTableByIntent"; 
	  int tenantid = 2; 
	  String callid = "test123";
	  String intent = "Anigroup donotdelete";
	  
	  GetIVRToTextTableByIntent test = new GetIVRToTextTableByIntent();
	  JSONObject response = test.start(url, callid, tenantid, intent, 10000, 10000, context);
