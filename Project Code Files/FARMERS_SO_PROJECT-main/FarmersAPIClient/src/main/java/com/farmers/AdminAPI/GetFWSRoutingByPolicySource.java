...
public class GetFWSRoutingByPolicySource {
	
	public JSONObject start(String url, String callid, int tenantid, String policysource, int conntimeout, int readtimeout, LoggerContext context) {
		Logger logger = context.getLogger(Constants.ADMIN_LOGGER_NAME);
		logger.info(callid + " : Params passed = "+url+" ,"+tenantid+", "+policysource+", "+conntimeout+", "+readtimeout);
		JSONObject resp = new JSONObject();
		JSONObject reqbody = new JSONObject();
		reqbody.put("tenantid", tenantid);
		reqbody.put("callerid", callid);
		reqbody.put("policysource", policysource);
	...
	private Map<String, String> getHeaders(Logger logger, String callid) {
      ...
      String url = "https://104.156.47.196/IVRAPI_UAT/api/FWSRouting/GetFWSRoutingByPolicySource";
	  //String url = "https://EC2AMAZ-ALDVADR/fiivrapi/api/AniGroup/CheckAniExistByKey"; 
	  int tenantid = 2; 
	  String policysource = "ARS";
	  String callid = "test123";
	  
	  GetFWSRoutingByPolicySource objGetFWSRoutingByPolicySource = new GetFWSRoutingByPolicySource();
	  JSONObject response = objGetFWSRoutingByPolicySource.start(url, callid, tenantid, policysource, 10000, 10000, context);
