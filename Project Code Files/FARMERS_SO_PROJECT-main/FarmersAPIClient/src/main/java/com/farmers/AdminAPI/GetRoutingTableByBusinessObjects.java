...
public class GetRoutingTableByBusinessObjects {
	
//private static final Logger logger = LogManager.getLogger(Constants.LOGGER_NAME);
	
	public JSONObject start(String url, String callid, int tenantid, String key, String dnis, String dnisgroup, String lob, String category, int conntimeout, int readtimeout, LoggerContext context) {
		
		Logger logger = context.getLogger(Constants.ADMIN_LOGGER_NAME);
		logger.info(callid+ " : Params passed = "+url+" ,"+tenantid+", "+key+", "+dnis+", "+dnisgroup+", "+lob+", "+category+", "+conntimeout+", "+readtimeout);
		...
		reqbody.put("lineofbusiness", lob);
	...
	/*  public static void main(String args[]) {
	  ...
	  String url = "https://104.156.46.196/IVRAPI/api/Routing/GetRoutingTableByBusinessObjects";
	  String callid = "9BD86499DDEA11EE9E80C48AF00346791";
	  int tenantid = 2; 
	  String dnis = "123456789"; 
	  String dnisgroup = "dnisgroup_donotdelete"; 
	  String lob = "lob_donotdelete"; 
	  String category = "category_donotdelete"; 
	  String key = "";
	  
	  GetRoutingTableByBusinessObjects test = new GetRoutingTableByBusinessObjects();
	  JSONObject response = test.start(url, callid, tenantid, key, dnis, dnisgroup, lob, category, 10000, 10000, context);
