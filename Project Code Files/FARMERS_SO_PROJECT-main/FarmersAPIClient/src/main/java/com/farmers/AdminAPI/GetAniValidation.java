...
public class GetAniValidation {
	
	public JSONObject start(String url, String callid, String ani, int conntimeout, int readtimeout, LoggerContext context) {
		Logger logger = context.getLogger(Constants.ADMIN_LOGGER_NAME);
		logger.info(callid+" : Params passed = "+url+" ,"+ani+", "+conntimeout+", "+readtimeout);
  ...
  /*	public static void main(String[] args) {
		  ...
		  String url = "https://104.156.46.196/IVRAPI/api/AniValidation/GetAniValidation"; 
		  String callid = "test123";
		  String ani = "1234567890";
		  
		  GetAniValidation test = new GetAniValidation();
		  JSONObject response = test.start(url, callid, ani, 10000, 10000, context);
	}  */
