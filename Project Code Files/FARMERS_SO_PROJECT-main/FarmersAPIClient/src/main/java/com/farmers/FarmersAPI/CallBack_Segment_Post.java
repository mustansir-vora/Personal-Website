...
public class CallBack_Segment_Post {

public JSONObject start(String url, String tid, int conntimeout, int readtimeout, String eventType ,String phoneNumber,String state,String timezone,String callBackTime,String callback,String callbackConsent,String firstName,String lastName, LoggerContext context) {
		
		Logger logger = context.getLogger(Constants.FARMERS_LOGGER_NAME);
		logger.info(tid+" : Parameters Passed from CVP : "+url+", "+tid+", "+eventType+", "+phoneNumber+", "+state+", "+timezone+", "+callBackTime+", "+callback+", "+callbackConsent+", "+firstName+", "+lastName+", "+conntimeout+", "+readtimeout);
