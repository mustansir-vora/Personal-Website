...
public class GetIVRToTextRecordings {
	
public JSONObject start(String url, String callid, int tenantid, int conntimeout, int readtimeout, LoggerContext context) {
		
		Logger logger = context.getLogger(Constants.ADMIN_LOGGER_NAME);
		logger.info(callid+" : Params passed = "+url+" ,"+tenantid+", "+conntimeout+", "+readtimeout);
	...
	private Map<String, String> getHeaders(Logger logger, String callid) {
	  ...
	  String url = "https://104.156.47.196/IVRAPI/api/IVRToText/GetIVRToTextRecordings"; 
	  int tenantid = 2; 
	  String callid = "test123";
	  
	  GetIVRToTextRecordings test = new GetIVRToTextRecordings();
	  JSONObject response = test.start(url, callid, tenantid, 10000, 10000, context);
