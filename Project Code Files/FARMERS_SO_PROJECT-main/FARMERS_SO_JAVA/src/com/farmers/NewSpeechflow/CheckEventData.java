...
public class CheckEventData extends DecisionElementBase {
	...
	public String doDecision(String elementName, DecisionElementData data) throws AudiumException {
		...
		try {
			...
			// Menu Counter NI/NM - Start

			data.addToLog(elementName, "=== START Menu Counter Processing ===");
			 
			// Get query text

			String queryText = (String) data.getElementData("VirtualAgentVoice", "query_text");
			// Get parameters
			

			String parameters = (String) data.getElementData("VirtualAgentVoice", "parameters");
			...
			if (parameters != null && !parameters.isEmpty()) {
			    try {
			        ...
			        parameterValue = (String) json.get("yesnoconf");
			        data.addToLog(elementName, "VirtualAgentVoice :" + parameters + " : " + parameterValue);
			    } catch (Exception e) {
			        data.addToLog(elementName, "Error parsing parameters: " + e.getMessage());
			    }
			}
			...
			data.addToLog(elementName, "Raw query_text from data: '" + queryText + "'");
			 
			if (queryText != null && !queryText.trim().isEmpty()) {

			    queryText = queryText.trim();

			    data.addToLog(elementName, "Processing query text: '" + queryText + "'");

			    // Get confidence score - handle both String and Float types

			    Float confidence = null;

			    Object confidenceObj = data.getElementData("VirtualAgentVoice", "confidence");

			    data.addToLog(elementName, "Raw confidence object: " + confidenceObj + " (type: " + (confidenceObj != null ? confidenceObj.getClass().getSimpleName() : "null") + ")");

			    if (confidenceObj instanceof Float) {

			        confidence = (Float) confidenceObj;

			    } else if (confidenceObj instanceof String) {

			        try {

			            confidence = Float.parseFloat((String) confidenceObj);

			            data.addToLog(elementName, "Parsed confidence from string: " + confidence);

			        } catch (NumberFormatException e) {

			            data.addToLog(elementName, "Failed to parse confidence string: '" + confidenceObj + "'");

			        }

			    } else if (confidenceObj instanceof Double) {

			        confidence = ((Double) confidenceObj).floatValue();

			        data.addToLog(elementName, "Converted confidence from double: " + confidence);

			    }

			    data.addToLog(elementName, "Final confidence score: " + confidence);

			    // Decision making logic - EVENT NAME FIRST

			    boolean shouldProcess = false;

			    String processReason = "Not processing";
			    String menuId = (String)data.getSessionData("S_MENU_ID_EVENT_NAME");

			    // FIRST PRIORITY: Check for specific events

			    if (menuId != null && !menuId.isEmpty()) {

			        if ("NLCL_MN_001".equalsIgnoreCase(menuId)) {

			            shouldProcess = true;

			            processReason = "menuId NLCL_MN_001 detected - will accumulate utterance";

			        } else if ("NLCL_MN_002".equalsIgnoreCase(menuId)) {

			            shouldProcess = true;

			            processReason = "menuId NLCL_MN_002 detected - will reset utterance";

			        } else {

			            processReason = "menuId '" + menuId + "' not in processing list (not NLCL_MN_001 or NLCL_MN_002)";

			        }

			    }

			    // SECOND PRIORITY: Check confidence score if no valid event found

			    // USING <= 0.49 as requested (for low confidence scenarios)
			    else if (menuId!=null &&confidence != null && "NLCL_MN_001".equalsIgnoreCase(menuId)&&(confidence < 0.50 || parameterValue.equalsIgnoreCase("Yes") || parameterValue.equalsIgnoreCase("No"))) {

			        shouldProcess = true;

			        processReason ="Menu Id from Menu Loader :"+menuId+" :"+ "Confidence score :" + confidence + " meets threshold (< 0.50) - or will accumulate utterance by yesnoconfirmation parameter";

			    } else {

			        processReason = "No valid Menu Id : "+menuId +":  and confidence score " + confidence + " above threshold (> 0.50)";

			    }

			    data.addToLog(elementName, "Processing decision: " + processReason);

			    if (shouldProcess) {

			        // Get existing utterance from session

			        String existingUtterance = (String) data.getSessionData("S_CALLER_UTTERANCE");

			        data.addToLog(elementName, "Current session utterance: '" + existingUtterance + "'");

			        // Handle NLCL_MN_002 event (reset logic)

			        if (menuId != null && "NLCL_MN_002".equalsIgnoreCase(menuId)) {

			            data.addToLog(elementName, "RESETTING utterance due to NLCL_MN_002 event");

			            data.setSessionData("S_CALLER_UTTERANCE", queryText);

			            data.addToLog(elementName, "Utterance reset to: '" + queryText + "'");

			        }

			        // Handle NLCL_MN_001 and confidence-based processing (accumulation logic)

			        else {

			            if (existingUtterance == null || "INITIAL".equalsIgnoreCase(existingUtterance)) {

			                data.addToLog(elementName, "Setting initial utterance (first time or INITIAL value)");

			                data.setSessionData("S_CALLER_UTTERANCE", queryText);

			                data.addToLog(elementName, "Initial utterance set: '" + queryText + "'");

			            } else {

			                data.addToLog(elementName, "Appending to existing utterance with pipe separator");

			                String newUtterance = existingUtterance + "|" + queryText;

			                data.setSessionData("S_CALLER_UTTERANCE", newUtterance);

			                data.addToLog(elementName, "Updated utterance: '" + newUtterance + "'");

			            }

			        }

			        // Verify and log final result

			        String finalUtterance = (String) data.getSessionData("S_CALLER_UTTERANCE");

			        data.addToLog(elementName, "FINAL utterance in session: '" + finalUtterance + "'");

			        data.addToLog(elementName, "FINAL utterance length: " +

			                         (finalUtterance != null ? finalUtterance.length() : 0));

			    } else {

			        data.addToLog(elementName, "SKIPPING processing - criteria not met");

			        data.addToLog(elementName, "Session utterance remains unchanged: '" +

			                         data.getSessionData("S_CALLER_UTTERANCE") + "'");
			    ...
			    if (queryText == null) {
			        ...
			        data.addToLog(elementName, "SKIPPING - query_text is NULL");

			    } else {

			        data.addToLog(elementName, "SKIPPING - query_text is EMPTY: '" + queryText + "'");

			    }

			}
			 
			data.addToLog(elementName, "=== END Menu Counter Processing ===");

			// Menu Counter NI/NM - End
			...
			if(null!= data.getSessionData("S_HOTEVENT")&& "Y".equalsIgnoreCase((String)data.getSessionData("S_HOTEVENT")) 
				...
				data.setSessionData("S_HOTEVENT", "N");	
				data.addToLog("Setting HOTEVENT Flag to :",(String) data.getSessionData("S_HOTEVENT"));
				...
				if(menuIDMode!=null && "FallBackDTMF".equals(menuIDMode.trim())){
					...
					data.setSessionData(Constants.value, "error");
					return "retry";
				}else {
