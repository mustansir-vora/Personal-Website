...
/*EM_ADM_002*/
public class EPCBrandsTable extends DecisionElementBase {
	...
	public String doDecision(String currElementName, DecisionElementData data) throws AudiumException {
		...
		try {
			if (data.getSessionData(Constants.S_EPC_BRANDS_URL) != null && data.getSessionData(Constants.S_DNIS) != null
				...
				JSONObject resp = (JSONObject) apiObj.start(url, callerId, Constants.tenantid, dnis,
				...
				if (resp != null) {
					...
					if (resp.containsKey(Constants.RESPONSE_CODE) && (int) resp.get(Constants.RESPONSE_CODE) == 200) {
						data.addToLog(currElementName, "Set EPCBrands API Response into session with the key name of "
						...
						if (resp.containsKey(Constants.RESPONSE_BODY)) {
							if (((JSONObject) resp.get(Constants.RESPONSE_BODY)).containsKey(("res"))) {
								JSONArray resArr = (JSONArray) ((JSONObject) resp.get(Constants.RESPONSE_BODY))
										.get("res");

								if (null != resArr && resArr.size() > 0) {

									data.setSessionData("S_SSAAH_MN_001_BRAND",
											((JSONObject) resArr.get(0)).get("key").toString());
									data.addToLog(currElementName, "S_SSAAH_MN_001_BRAND ::"
											+ ((JSONObject) resArr.get(0)).get("key").toString());
								}
							}
						}
