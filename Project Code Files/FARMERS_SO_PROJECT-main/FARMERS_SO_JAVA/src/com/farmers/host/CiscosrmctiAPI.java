...
/*SPT_HOST_001*/
public class CiscosrmctiAPI extends DecisionElementBase {
	...
	public String doDecision(String currElementName, DecisionElementData data) throws AudiumException {
		...
		if(data.getSessionData(Constants.S_CISCO_SRMCTI_URL) != null && data.getSessionData(Constants.S_CONN_TIMEOUT) != null
			...
			if("03".equalsIgnoreCase(poptype)) {
				callerinput = (String) data.getSessionData(Constants.S_CALLER_INPUT);
				data.addToLog("caller input :", callerinput);
			}
			data.addToLog("caller input :", callerinput);
			...
			data.addToLog(currElementName, "brand : "+brand);
			...
			//JSONObject resp =  (JSONObject) obj.start(url, callid,  callerani, destinationapp, tollfreenumber, callerinput, poptype, calledinto, calledintime, tollfreedescription, callertype, language, an1, upn1, transferreason, intent , brand , validpolicynumber, callerrisklevel, conTimeout, readTimeout, context);
			...
			if("YES".equalsIgnoreCase(UAT_FLAG)) {
				...
				resp = objNP.start(url, callid,  callerani, destinationapp, tollfreenumber, callerinput, poptype, calledinto, calledintime,
			...
			data.addToLog(currElementName, "CiscosrmctiAPI response  :"+resp);
		...
		data.addToLog(currElementName,"Exception in CiscosrmctiAPI call  :: "+e);
		caa.printStackTrace(e);
	}
	
		try {
			objHostDetails.startHostReport(currElementName,"CiscosrmctiAPI", strReqBody,region, (String) data.getSessionData(Constants.S_CISCO_SRMCTI_URL));
