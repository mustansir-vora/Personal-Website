...
/*KYCBA_HOST_001*/
public class PolicyInquiryAPI_NEW extends DecisionElementBase {
	...
	public String doDecision(String currElementName, DecisionElementData data) throws AudiumException {
		...
		String strRespBody = data.getSessionData("S_POLICYAPI_RESPONSE").toString();
		data.addToLog("API Response: ", strRespBody);
		...
		try {
			...
			case Constants.S_API_BU_BW:
				...
				StrExitState=apiResponseManupulation_RetriveInsurancePoliciesByParty(data, caa, currElementName, strRespBody, callerenteredDOB);
				break;
			case Constants.S_API_BU_FDS_PLA:
				...
				StrExitState=apiResponseManupulation_MulesoftFarmerPolicyInquiry(data, caa, currElementName, strRespBody,callerenteredDOB);
				break;
			...
			case Constants.S_API_BU_FM:
				...
				StrExitState=apiResponseManupulation_ForemostPolicyInquiry(data, caa, currElementName, strRespBody,callerenteredDOB);
				break;
			case Constants.S_API_BU_FWS_ARS:
				...
				StrExitState=apiResponseManupulation_FwsPolicyLookup(data, caa, currElementName, strRespBody,callerenteredDOB);
				break;
			...
			case Constants.S_API_BU_21C:
				...
				StrExitState=apiResponseManupulation_PointPolicyInquiry(data, caa, currElementName, strRespBody, callerenteredDOB);
				break;
			default:
				break;
			}
...
//		}
	...
	private String apiResponseManupulation_RetriveInsurancePoliciesByParty(DecisionElementData data, CommonAPIAccess caa, String currElementName, String strRespBody, String callerenteredDOB) {
		...
		try {
			...
			String concatenatedPolicyDOB = Constants.EmptyString;
						...
						for(Object objPolicies : autopolicyArray) {
							...
							if (null != policyobj && !policyobj.isEmpty() && policyobj.containsKey("namedInsured")) {
								...
								if (null != namedinsured && !namedinsured.isEmpty() && namedinsured.containsKey("birthDate")) {
									...
									if (policyDOB.contains("/")) {
										policyDOB = resultSDF.format(inputSDF.parse(policyDOB));
										concatenatedPolicyDOB = (concatenatedPolicyDOB.equals("")) ? "" + policyDOB
												: concatenatedPolicyDOB + "," + policyDOB;
									} else {
										concatenatedPolicyDOB = (concatenatedPolicyDOB.equals("")) ? "" + policyDOB
												: concatenatedPolicyDOB + "," + policyDOB;
									}
									data.addToLog(currElementName, "API DOB  in  RetriveInsurancePoliciesByParty:: "+concatenatedPolicyDOB);
	...
	private String apiResponseManupulation_MulesoftFarmerPolicyInquiry(DecisionElementData data, CommonAPIAccess caa, String currElementName, String strRespBody,String callerenteredDOB) {
		String strExitstate=Constants.SU;
		try {
			JSONObject resp = (JSONObject) new JSONParser().parse(strRespBody);
					...
					for(Object policyArrIterator : policiesArr) {
						...
						if (null != policydata && !policydata.isEmpty()) {
							...
							if (policydata.get("payPlan") != null &&  policydata.containsKey("payPlan")) {
								...
								data.addToLog(currElementName, "S_FDS_PAYROLL_DEDUCT : " + (String) data.getSessionData("S_FDS_PAYROLL_DEDUCT"));
									...
									for(Object insuredDetailsArrayIterator : insuredDetailsArray) {
										...
										if (null != insuredDetails) {
											...
											if (insuredDetails.containsKey("birthDate")) {
												...
												if (policyDOB.contains("/")) {
													policyDOB = resultSDF.format(inputSDF.parse(policyDOB));
													concatenatedPolicyDOB = (concatenatedPolicyDOB.equals(""))
															? "" + policyDOB
															: concatenatedPolicyDOB + "," + policyDOB;
												} else {
													concatenatedPolicyDOB = (concatenatedPolicyDOB.equals(""))
												...
												data.addToLog(currElementName, "Date Received from API after concatination in MulesoftFarmerPolicyInquiry:: "+concatenatedPolicyDOB);
												String[] splitDOB = concatenatedPolicyDOB.split(",");
												for(int singleSplit = 0; singleSplit < splitDOB.length;singleSplit++) {
													policyDOB = splitDOB[singleSplit];
												
												if (callerenteredDOB.equalsIgnoreCase(policyDOB)) {
													
													data.setSessionData(Constants.S_API_DOB, concatenatedPolicyDOB);
								...
								if (null != addressArray && addressArray.size() > 0) {
									...
									if (null != addressobj) {
										...
										if (addressobj.containsKey("streetAddress1")) {
											...
											data.addToLog(currElementName, "streetAddress1: " + address);
										}
										data.addToLog(currElementName, "Enter country check ");
//										if (addressobj.containsKey("country") && addressobj.get("country")!=null) { 
//											data.addToLog(currElementName, "Inside country check ");									
...
//										}
										...
										Object countryObj = addressobj.get("country");
										   if (countryObj != null) {
										       ...
										       data.addToLog(currElementName, "Country is null");
										   }
										}
									}
					...
					if (null == data.getSessionData(Constants.S_API_DOB)) {
						...
						if (null != policydata && policydata.containsKey("insuredDetails")) {
							...
							if (null != insuredDetails && insuredDetails.containsKey("birthDate")) {
								...
								concatenatedPolicyDOB = (concatenatedPolicyDOB.equals(""))?""+policyDOB:concatenatedPolicyDOB+","+policyDOB;
								data.setSessionData(Constants.S_API_DOB, concatenatedPolicyDOB);
								data.setSessionData(Constants.S_FINAL_POLICY_OBJ, policydata);
								data.setSessionData(currElementName+Constants._RESP, policydata);
							}
	...
	private String apiResponseManupulation_ForemostPolicyInquiry(DecisionElementData data, CommonAPIAccess caa, String currElementName, String strRespBody,String callerenteredDOB) {
		String strExitState=Constants.SU;
		try {
			JSONObject resp = (JSONObject) new JSONParser().parse(strRespBody);
			JSONObject policydata = new JSONObject();
...
//											data.addToLog(currElementName, "Date Match : API Date Recieved :: "+policyDOB);
								...
								data.setSessionData(Constants.S_API_DOB, concatenatedPolicyDOB);
								data.addToLog(currElementName, "All DOB's Concatination in ForemostPolicyInquiry::"+data.getSessionData(Constants.S_API_DOB));
								if (null == data.getSessionData(Constants.S_API_DOB)) {
									for (Object objpolicies : insuredDetailsArr) {
	...
	private String apiResponseManupulation_FwsPolicyLookup(DecisionElementData data, CommonAPIAccess caa, String currElementName, String strRespBody,String callerenteredDOB) {
		String strExitState=Constants.SU;
		try {
			JSONObject resp = (JSONObject) new JSONParser().parse(strRespBody);
						...
						if (null != policyobj && policyobj.containsKey("insuredDetails")) {
							...
							for (Object insuredDetailsobj : insuredDetailsArr) {
								...
								data.addToLog(currElementName,
										"Date Received from API in FwsPolicyLookup:: " + policyDOB);
								//if (!policysource.isEmpty() && policysource.equalsIgnoreCase("ARS")) {
									...
									data.addToLog(currElementName,
											"Date After Conversion in FwsPolicyLookup :: " + concatenatedPolicyDOB);
								//}
								
								//CS1248449 - "FWSSharedBillingAPIFailure" when the customer selects Billing & Payments from the Shared Main Menu for FWS Service policies.
							...
							if (null == data.getSessionData(Constants.S_API_DOB) && null != insuredDetailsArr && insuredDetailsArr.size() > 0) {
								...
								if (!policysource.isEmpty() && policysource.equalsIgnoreCase("ARS")) {
									...
									String[] splitDOB = concatenatedPolicyDOB.split(",");
									for(int singleSplit = 0; singleSplit < splitDOB.length;singleSplit++) {
										policyDOB = resultSDF.format(inputSDF.parse(splitDOB[singleSplit]));
									}
									concatenatedPolicyDOB = (concatenatedPolicyDOB.equals(""))?""+policyDOB:concatenatedPolicyDOB+","+policyDOB;
									data.addToLog(currElementName, "Date After Conversion in FwsPolicyLookup :: "+concatenatedPolicyDOB);
								}
	...
	private String apiResponseManupulation_PointPolicyInquiry(DecisionElementData data, CommonAPIAccess caa, String currElementName, String strRespBody, String callerenteredDOB) {
		String strExitState=Constants.SU;
