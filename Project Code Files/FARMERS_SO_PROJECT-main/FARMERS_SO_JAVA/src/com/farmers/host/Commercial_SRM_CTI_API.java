...
import com.farmers.FarmersAPI.Commercialsrmcti_Post;
import com.farmers.FarmersAPI_NP.Commercialsrmcti_NP_Post;
import com.farmers.report.SetHostDetails;
import com.farmers.util.CommonAPIAccess;
import com.farmers.util.Constants;

public class Commercial_SRM_CTI_API extends DecisionElementBase {
	...
	public String doDecision(String currElementName, DecisionElementData data) throws AudiumException {
		...
		try {
			if(data.getSessionData(Constants.S_COMMERCIAL_SRMCTI_URL) != null && data.getSessionData(Constants.S_CONN_TIMEOUT) != null && data.getSessionData(Constants.S_READ_TIMEOUT) != null) {
				/** URL **/
				String url = (String) data.getSessionData(Constants.S_COMMERCIAL_SRMCTI_URL);
				...
				//START : CS1158163 : Farmers Insurance | US | Commercial - Missing Brand in Screenpop
				
				data.addToLog("Brand Name : ", brand);
	            brand = (String) data.getSessionData(Constants.S_BRAND);
	            data.setSessionData("S_BRAND", brand);
	            data.addToLog("Brand Name :::", brand);
				
	          //END : CS1158163 : Farmers Insurance | US | Commercial - Missing Brand in Screenpop
				
	            
				/** Caller Input**/
				//Policy Number, Account number, SR number
				...
				Commercialsrmcti_Post obj = new Commercialsrmcti_Post();
				//JSONObject resp =  (JSONObject) obj.start(url, callid, callerani, destinationapp, tollfreenumber,callerinput,poptype,  calledinto, calledintime, 
						//tollfreedescription, callertype, language, an1, upn1, transferreason, intent, brand, validpolicynumber, callerrisklevel, conTimeout, readTimeout, 
						//context); 
				...
				Commercialsrmcti_NP_Post npObj=new Commercialsrmcti_NP_Post();
		
				JSONObject resp=null ;
				if("YES".equalsIgnoreCase(UAT_FLAG)) {
					String Key =Constants.S_COMMERCIAL_SRMCTI_URL;
				...
				data.addToLog(currElementName, "Commercial_SRM_CTI_API response  :"+resp);
				//Mustan - Alerting Mechanism ** Response Code Capture
			...
			data.addToLog(currElementName,"Exception in Commercial_SRM_CTI_API call  :: "+e);
			caa.printStackTrace(e);
		}

		try {
			objHostDetails.startHostReport(currElementName,"Commercial_SRM_CTI_API", strReqBody, region,(String) data.getSessionData(Constants.S_COMMERCIAL_SRMCTI_URL));
			objHostDetails.endHostReport(data,strRespBody , StrExitState.equalsIgnoreCase(Constants.ER) ? Constants.ER : Constants.SU, apiRespCode,"");
		} catch (Exception e) {
			data.addToLog(currElementName,"Exception while forming host reporting for Commercial_SRM_CTI_API call  :: "+e);
			caa.printStackTrace(e);
		}
