...
public class MulesoftFarmersPolicyInquiry_BC extends DecisionElementBase {
					...
					if (resp.containsKey(Constants.RESPONSE_CODE) && (int) resp.get(Constants.RESPONSE_CODE) == 200
						...
						if (respObj.containsKey("policies")) {
							...
							if (null != agentsArr && agentsArr.size() > 0) {
								...
								data.setSessionData(Constants.S_POLICY_SOURCE, ((JSONObject) agentsArr.get(0)).get("policySource"));
								data.addToLog(currElementName,
								...
								lob = (String) data.getSessionData("S_LOB");
								if(lob!=null && !Constants.EmptyString.equalsIgnoreCase(lob)){
									if("A".equalsIgnoreCase(lob)) {
										caa.createMSPKey(caa, data, "FASMM_MN_001", "AUTO");
									}else if("F".equalsIgnoreCase(lob)) {
										caa.createMSPKey(caa, data, "FASMM_MN_001", "HOME");
									}else if("U".equalsIgnoreCase(lob)) {
										caa.createMSPKey(caa, data, "FASMM_MN_001", "UMBRELLA");
									}else if("P".equalsIgnoreCase(lob)) {
										caa.createMSPKey(caa, data, "FASMM_MN_001", "UMBRELLA");
									}else {
										// Nthg to be set
									}
								}
								StrExitState = Constants.POLICY_FOUND;

							} else {
								data.setSessionData("S_MULESOFT_POLICY_FOUND", "FALSE");
								data.addToLog(currElementName, "Value of S_MULESOFT_POLICY_FOUND : "+ (String) data.getSessionData("S_MULESOFT_POLICY_FOUND"));
								StrExitState = Constants.POLICY_NOT_FOUND;
							}
						} else {
							StrExitState = Constants.POLICY_NOT_FOUND;
						}
