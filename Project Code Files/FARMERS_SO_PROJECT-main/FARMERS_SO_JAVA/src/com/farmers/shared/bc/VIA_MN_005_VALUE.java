...
public class VIA_MN_005_VALUE extends DecisionElementBase  {
	...
	public String doDecision(String currElementName, DecisionElementData data) throws AudiumException {
		...
		try {
			...
			String returnValue = (String) data.getElementData("VIA_MN_005_Call","Return_Value");
			data.addToLog(currElementName, "Menu ID : "+"VIA_MN_005"+" :: Menu Value : "+returnValue);
			data.setSessionData("VIA_MN_005_VALUE", returnValue);
...
//			}
			if (strAPI_DOB != null) {
			    String[] arrAPIDOB = strAPI_DOB.split(",");
			    StringBuilder finalDOBBuilder = new StringBuilder();
			    ...
			    for (String temp : arrAPIDOB) {
			        ...
			        try {
			            if (temp.contains("/")) {
			                ...
			                // It's already in yyyy-MM-dd, so parse and reformat
			                SimpleDateFormat inputSDF = new SimpleDateFormat("yyyy-MM-dd");
			                SimpleDateFormat resultSDF = new SimpleDateFormat("MM-dd-yyyy");
			                temp = resultSDF.format(inputSDF.parse(temp));
			            }
			            if (finalDOBBuilder.length() > 0) {
			                finalDOBBuilder.append(",");
			            }
			            finalDOBBuilder.append(temp);
			            data.addToLog(currElementName, "Final Appended Date String After Format conversion :: " + finalDOBBuilder.toString());
			        } catch (ParseException e) {
			            data.addToLog(currElementName, "Error parsing date: " + temp + " - " + e.getMessage());
			        }
			    }
			    finalDOB = finalDOBBuilder.toString();
			}
			...
			if(returnValue.equalsIgnoreCase(Constants.NOINPUT)) {
				...
				data.setSessionData("LAST_MENU_NAME_REP", "VIA_MN_005");
			}
				...
				if(returnValue.equalsIgnoreCase("I Dont Know")) {
					...
					if(strMenuValue.equalsIgnoreCase(finalDOB)||finalDOB.contains(strMenuValue)) {
						...
						data.setSessionData("S_CALLER_AUTH", "Authenticated");
			...
			data.addToLog(currElementName,"Exception in VIA_MN_005 :: "+e);
			caa.printStackTrace(e);
		}
		data.addToLog(currElementName,"VIA_MN_005 :: "+strExitState);
		data.setSessionData("VIA_MN_005_ExitState", strExitState);
		return strExitState;
	}
