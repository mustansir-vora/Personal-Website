...
public class FWS_SBP_MN_004 extends DecisionElementBase 
{
	@Override
	public String doDecision(String currElementName, DecisionElementData data) throws AudiumException 
	{
		...
		try {
			...
			String strReturnValue = (String) data.getElementData("FWS_SBP_MN_004_Call","Return_Value");
			data.addToLog(currElementName, "Menu ID : "+"FWS_SBP_MN_004"+" :: Menu Value : "+strReturnValue);
			if(null != strReturnValue && strReturnValue.contains("$")) {
				strReturnValue = strReturnValue.replace("$", "");
				data.addToLog(currElementName, "Menu ID : "+"FWS_SBP_MN_004"+" :: Menu Value post $ replace : "+strReturnValue);
			}
			if (null != strReturnValue && strReturnValue.contains(",")) {
				strReturnValue = strReturnValue.replace(",", "");
				data.addToLog(currElementName, "Menu ID : "+"FWS_SBP_MN_004"+" :: Menu Value post , replace : "+strReturnValue);
			}
			data.setSessionData("FWS_SBP_MN_004_VALUE", strReturnValue);
			if(strReturnValue != null && !strReturnValue.equals("")) {
				if(strReturnValue.equalsIgnoreCase(Constants.NOINPUT)) {
					strExitState = Constants.NOINPUT;
				}else if(strReturnValue.equalsIgnoreCase(Constants.NOMATCH)) {
					strExitState = Constants.NOMATCH;
				}else if (strReturnValue.equalsIgnoreCase("Minimum Due") || strReturnValue.equalsIgnoreCase("MinumumDue")) {
					data.setSessionData("S_Current_Balance", data.getSessionData("S_MINIMUMDUE"));
					data.addToLog(currElementName, "Overriding Current Balance for PaymentUS :: "+ data.getSessionData("S_MINIMUMDUE"));
					...
					data.addToLog(currElementName, "Overriding Current Balance for PaymentUS :: "+ data.getSessionData("S_FULLBALANCE"));
					strExitState = "Full Balance";
				} else {
					strExitState = "invalid";
					
					if (null != strReturnValue && strReturnValue.length() <= 2) {
						return strExitState;
					}
					
					if (null != strReturnValue && !strReturnValue.contains(".") && strReturnValue.length() > 2) {
						strReturnValue = strReturnValue.substring(0, strReturnValue.length()-2)+"."+ strReturnValue.substring(strReturnValue.length()-2, strReturnValue.length());
						data.addToLog(currElementName, "Return Value Post Modifications :: " + strReturnValue);
					}
					...
					try {
						...
						if(m.find()) {
							...
							String nextPaymentAmount = (String)data.getSessionData("Full_BALANCE_DUE");
							...
							data.addToLog(currElementName, "post double conversion : nextPaymentAmount : "+amount+" FWS_SBP_MN_004 inputAmt : "+inputAmt);
							if(null != amount && null != inputAmt && inputAmt >= 1.00 && inputAmt <= amount) {
								data.setSessionData("Valid_Get_Payment", "TRUE");
								data.setSessionData("S_Current_Balance", inputAmt.toString());
								data.addToLog(currElementName, "Overriding Current Balance for PaymentUS :: "+ inputAmt);
								strExitState = "valid";
