...
public class BillPayments extends DecisionElementBase {
	...
	private String billpresentmentAfterHalo(DecisionElementData data, CommonAPIAccess caa, String currElementName, boolean isLastPaymentDetailAvail) {
		...
		try {
			if ((data.getSessionData(Constants.S_BILL_PRESENTMENT_RETRIEVE_BILLINGSUMMARY_URL) != null)
				...
				//UAT  END ENV CHANGE Start(Priya,Shaik)
				data.addToLog("responses", responses.toString());
	...
	private String pointgeneralbilling(DecisionElementData data, CommonAPIAccess caa, String currElementName) {
		...
		try {
			...
			if ((data.getSessionData(Constants.S_POINTGENERAL_BILLING_URL) != null)
				...
				JSONObject responses = lookups.Getpointgeneralbilling(wsurl, tid,Integer.valueOf(Integer.parseInt(conntimeout)), Integer.valueOf(Integer.parseInt(readtimeout)),context,region,UAT_FLAG);
				data.addToLog("responses", responses.toString());
			...
			if ((nextPaymentDate != null) && (!nextPaymentDate.equals("")) && (nextPaymentAmount != null)
				...
				data.setSessionData(Constants.S_NEXTPAYMENT_AMOUNT, nextAmount);
				 ...
				 Instant instant = Instant.now();
					...
					Date systemDate = sdf.parse(new SimpleDateFormat(Constants.PAYMENT_DATE_FORMAT).format(centralTime));
			        
			        String convertedcentralTimeZoneDate=sdf.format(systemDate);
	...
	private String apiResponseManupulation_FWSBillingLookup(DecisionElementData data, CommonAPIAccess caa,
		...
		try {
			...
			if (policysource.equalsIgnoreCase("ARS")) {
				...
				if(null == ErroCode || ErroCode.isEmpty() || !ErroCode.equalsIgnoreCase("TRANSFER") || !ErroCode.equalsIgnoreCase("NPAC") || 
						!ErroCode.equalsIgnoreCase("NPR") || !ErroCode.equalsIgnoreCase("R0") || !ErroCode.equalsIgnoreCase("R0") || 
						!ErroCode.equalsIgnoreCase("PB8") || !ErroCode.equalsIgnoreCase("PB9") || !ErroCode.equalsIgnoreCase("PB1")
						 || !ErroCode.equalsIgnoreCase("PB7")) {
					data.setSessionData("S_NEXT_PAYMENT_ALLOWED", Constants.YES);
					data.addToLog(currElementName, "Is Next Payment Allowed for ARS policy source: "+data.getSessionData("S_NEXT_PAYMENT_ALLOWED"));
				}
				...
				if (billsummary.getTerms() != null) {
					...
					if(null != objTerms.getPaymentAllowed() &&  Constants.YES.equalsIgnoreCase( objTerms.getPaymentAllowed())) {
						data.setSessionData("S_NEXT_PAYMENT_ALLOWED", Constants.YES);
						data.addToLog(currElementName, "Is Next Payment Allowed for Met360: "+data.getSessionData("S_NEXT_PAYMENT_ALLOWED"));
					}
				}
