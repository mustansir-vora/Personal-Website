...
public class NEWMDMPhoneNoLookup extends DecisionElementBase {
...
//			String str21stCode = (String) data.getApplicationAPI().getApplicationData("A_KYCBA_21ST_LOB");
	...
	private String apiResponseManupulation_MdmPhoneNoPolicyLookup(DecisionElementData data, CommonAPIAccess caa,
		...
		String strPolicyStatus = Constants.EmptyString, appTag = Constants.EmptyString;
...
//			if (policiesArr.size() > 5) {
//				data.addToLog(currElementName, "policies size is greater than 5 started sorting....");
//				policiesArr = filterPoliciesByInceptionDate(policiesArr, data);
//				data.addToLog(currElementName, "sorted array by inception date");
//			}
			data.addToLog(currElementName, "poliices array ::" + policiesArr.toJSONString());
...
//					}
					...
					//CS1360621-Add 'IA' Routing Qualifier
					if (policyData.containsKey("policyCategory") && null != policyData.get("policyCategory"))
						beanObj2.setpolicyCategory((String) policyData.get("policyCategory"));
					//---------
					...
					if (policyData.containsKey("inceptionDate") && policyData.get("inceptionDate") != null);
					
					{
						String inceptionDate = formatDate(data, (String) policyData.get("inceptionDate"),
								"yyyy-MM-dd HH:mm:ss.S", "MM/dd/yyyy");
						beanObj2.setInceptionDate(inceptionDate);
						data.addToLog(currElementName, "setInceptionDate :" + beanObj2.getInceptionDate());
						//data.setSessionData("S_MDM_INCEPTION_DATE", inceptionDate);
					}
				}
				...
				if (policiesArr.size() == 1) {
					...
					for (String key : productPolicyMap.keySet()) {
						...
						for (String key1 : policyDetailsMap.keySet()) {
							...
							data.setSessionData(Constants.CCAI_Customer_Policy_Number, key1);
					   ...
					   // if( apponBoardingFlag.equalsIgnoreCase(Constants.Y_FLAG)) {
						String mdmInceptionDate= obj.getInceptionDate();
						data.addToLog("Inception date is: ", mdmInceptionDate);
						...
						data.addToLog("The MDM Inception Date is "+mdmInceptionDate, "The Current date is "+formattedDate);
						LocalDate date1= LocalDate.parse(mdmInceptionDate, formatter);
...
//							}
					...
					if (lang.equalsIgnoreCase("sp") && prompt.toLowerCase().contains(" your auto policy")) {
						...
						prompt = prompt.replace(" your Auto Policy", " su póliza de auto");
					}
					if (lang.equalsIgnoreCase("sp") && prompt.toLowerCase().contains(" your home policy")) {
						prompt = prompt.replace(" your Home Policy", " su póliza de hogar");
					}
					if (lang.equalsIgnoreCase("sp") && prompt.toLowerCase().contains(" your umbrella policy")) {
						prompt = prompt.replace(" your Umbrella Policy", " su póliza paraguas");
					}
					if (lang.equalsIgnoreCase("sp")
							&& prompt.toLowerCase().contains(" your marine watercraft policy")) {
						prompt = prompt.replace(" your Marine Watercraft Policy", " su póliza de embarcación marina");
					}
					if (lang.equalsIgnoreCase("sp") && prompt.toLowerCase().contains(" your travel trailer policy")) {
						prompt = prompt.replace(" your Travel Trailer Policy", " su póliza de remolque de viaje");
					}
					if (lang.equalsIgnoreCase("sp") && prompt.toLowerCase().contains(" your motor home policy")) {
						prompt = prompt.replace(" your Motor Home Policy", " su póliza de casa rodante");
					}
					if (lang.equalsIgnoreCase("sp") && prompt.toLowerCase().contains(" your motorcycle policy")) {
						prompt = prompt.replace(" your Motorcycle Policy", " su póliza de motocicleta");
					}
					if (lang.equalsIgnoreCase("sp") && prompt.toLowerCase().contains(" your off road vehicle policy")) {
						prompt = prompt.replace(" your Off Road Vehicle Policy", " su póliza de vehículo todoterreno");
					}
					if (lang.equalsIgnoreCase("sp")
							&& prompt.toLowerCase().contains(" your manufactured home policy")) {
						prompt = prompt.replace(" your Manufactured Home Policy", " su póliza de casa prefabricada");
					}
					if (lang.equalsIgnoreCase("sp")
	...
	private JSONArray filterDuplicatePoliciesByNumber(JSONArray policiesArr) {
		...
		for (int i = 0; i < policiesArr.size(); i++) {
			...
			if (policyNumber != null && lineOfBusiness != null) {
				String key = policyNumber + "|" + lineOfBusiness;
	...
	public JSONArray filterPoliciesByInceptionDate(JSONArray policies, DecisionElementData data) {
	    ...
	    if (policies != null) {
	        for (int i = 0; i < policies.size(); i++) {
	            ...
	            if (inceptionDateStr != null && !inceptionDateStr.isEmpty()) {
	                data.addToLog(data.getCurrentElement(),
	                        "Inception date present in policy: " + inceptionDateStr);
	                try {
	                    // Parse the inception date
	                    Date inceptionDate = sdf.parse(inceptionDateStr);
	                    // Store the parsed date in the policy object for sorting
	                ...
	                data.addToLog(data.getCurrentElement(),
	                        "Inception date field is not present in policy: " + policy);
	            }
	        }

	        // Sort policies by descending inceptionDate
	        ...
	        // Add all policies to the result array
	        for (JSONObject policy : policyList) {
	            latestPoliciesArray.add(policy);
	        }
	    } else {
	        data.addToLog(data.getCurrentElement(), "Policies array is null");
	    }
