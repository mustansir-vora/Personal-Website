...
public class SharedIDAuthPhone extends DecisionElementBase 
	...
	public String doDecision(String currElementName, DecisionElementData data) throws AudiumException 
	{
		...
		try {
			...
			if(strBristolCode!=null && strBU!=null && strBristolCode.contains(strBU)) {
				StrExitState = policyInquiry_RetriveInsurancePoliciesByParty(strReqBody, strRespBody, data, caa, currElementName, StrExitState);
				return StrExitState;
			}
			else if(strFarmersCode!=null && strBU!=null && strFarmersCode.contains(strBU))
			{
				StrExitState = mulesoftFarmerPolicyInquiry(strReqBody, strRespBody, data, caa, currElementName, StrExitState);
				return StrExitState;
			}
			else if(strForemostCode!=null && strBU!=null && strForemostCode.contains(strBU)) 
			{
				StrExitState = foremostPolicyInquiry(strReqBody, strRespBody, data, caa, currElementName, StrExitState);
				return StrExitState;
			}
			else if(strFWSCode!=null && strBU!=null && strFWSCode.contains(strBU)) 
			{
				StrExitState = fwsPolicyLookup(strReqBody, strRespBody, data, caa, currElementName, StrExitState);
				return StrExitState;
			}
			else if(str21stCode!=null && strBU!=null && str21stCode.contains(strBU))
			{
				StrExitState = PointPolicyInquiry(strReqBody, strRespBody, data, caa, currElementName, StrExitState);
				return StrExitState;
			}	
			else
			{
				StrExitState = AccountLinkANILookup(strReqBody, strRespBody, data, caa, currElementName, StrExitState);
				return StrExitState;
			}
...
*/
		...
		{
			data.addToLog(currElementName,"Exception while forming host details for SIDA_HOST_004  :: "+e);
			caa.printStackTrace(e);
		}

		/*
		try {
			objHostDetails.startHostReport(currElementName,"SIDA_HOST_003", strReqBody);
			objHostDetails.endHostReport(data,strRespBody , StrExitState.equalsIgnoreCase(Constants.ER) ? Constants.ER : Constants.SU);
		} catch (Exception e) {
			data.addToLog(currElementName,"Exception while forming host reporting for SIDA_HOST_003 call  :: "+e);
			caa.printStackTrace(e);
		}
		*/
		return StrExitState;
	...
	private String AccountLinkANILookup(String strReqBody, String strRespBody, DecisionElementData data,
			CommonAPIAccess caa, String currElementName, String strExitState) 
	{
		...
		try 
		{
			if(data.getSessionData(Constants.S_ACCOUNTLINKANILOOKUPURL) != null )
			{
				...
				String UAT_FLAG= (String)data.getSessionData(Constants.IS_UAT_DNIS);
		...
		try {
			objHostDetails.startHostReport(currElementName,"ACElookupAPI", strReqBody,region, (String) data.getSessionData(Constants.S_ACCOUNTLINKANILOOKUPURL));
			objHostDetails.endHostReport(data,strRespBody , strExitState.equalsIgnoreCase(Constants.ER) ? Constants.ER : Constants.SU, apiRespCode,"");
		} catch (Exception e) {
			data.addToLog(currElementName,"Exception while forming host reporting for ACElookupAPI call  :: "+e);
			caa.printStackTrace(e);
		}
		return strExitState;

	}
	private String PointPolicyInquiry(String strReqBody, String strRespBody, DecisionElementData data,
			CommonAPIAccess caa, String currElementName, String strExitState) 
	{
		SetHostDetails objHostDetails = new SetHostDetails(caa);objHostDetails.setinitalValue();
		...
		try {
			objHostDetails.startHostReport(currElementName,"PointPolicyInquiry", strReqBody,region, (String) data.getSessionData(Constants.S_POINT_POLICYINQUIIRY_URL));
			objHostDetails.endHostReport(data,strRespBody , strExitState.equalsIgnoreCase(Constants.ER) ? Constants.ER : Constants.SU, apiRespCode,"");
		} catch (Exception e) {
			data.addToLog(currElementName,"Exception while forming host reporting for PointPolicyInquiry call  :: "+e);
			caa.printStackTrace(e);
		}
		return strExitState;
	}

	private String fwsPolicyLookup(String strReqBody, String strRespBody, DecisionElementData data,
			CommonAPIAccess caa, String currElementName, String strExitState) 
	{
		...
		try {
			objHostDetails.startHostReport(currElementName,"FWSPolicyLookup", strReqBody,region, (String) data.getSessionData(Constants.S_FWS_POLICYLOOKUP_URL));
			objHostDetails.endHostReport(data,strRespBody , strExitState.equalsIgnoreCase(Constants.ER) ? Constants.ER : Constants.SU, apiRespCode,"");
		} catch (Exception e) {
			data.addToLog(currElementName,"Exception while forming host reporting for FWSPolicyLookup call  :: "+e);
			caa.printStackTrace(e);
		}
	...
	private String foremostPolicyInquiry(String strReqBody, String strRespBody, DecisionElementData data,
			CommonAPIAccess caa, String currElementName, String strExitState)
	{
		SetHostDetails objHostDetails = new SetHostDetails(caa);objHostDetails.setinitalValue();
		...
		try {
			objHostDetails.startHostReport(currElementName,"ForemostPolicyInquiry", strReqBody, region,(String) data.getSessionData(Constants.S_FOREMOST_POLICYINQUIIRY_URL));
			objHostDetails.endHostReport(data,strRespBody , strExitState.equalsIgnoreCase(Constants.ER) ? Constants.ER : Constants.SU, apiRespCode,"");
		} catch (Exception e) {
			data.addToLog(currElementName,"Exception while forming host reporting for ForemostPolicyInquiry call  :: "+e);
			caa.printStackTrace(e);
		}
	...
	private String mulesoftFarmerPolicyInquiry(String strReqBody, String strRespBody, DecisionElementData data,
			CommonAPIAccess caa, String currElementName, String strExitState) 
	{		
		SetHostDetails objHostDetails = new SetHostDetails(caa);objHostDetails.setinitalValue();
		//Mustan - Alerting Mechanism ** Response Code Capture
		...
		try {
			objHostDetails.startHostReport(currElementName,"MulesoftFarmerPolicyInquiry_Post", strReqBody, region,(String) data.getSessionData(Constants.S_MULESOFT_FARMER_POLICYINQUIRY_URL));
			objHostDetails.endHostReport(data,strRespBody , strExitState.equalsIgnoreCase(Constants.ER) ? Constants.ER : Constants.SU, apiRespCode,"");
		} catch (Exception e) {
			data.addToLog(currElementName,"Exception while forming host reporting for MulesoftFarmerPolicyInquiry_Post  :: "+e);
			caa.printStackTrace(e);
		}
	...
	private String policyInquiry_RetriveInsurancePoliciesByParty(String strReqBody, String strRespBody,
			DecisionElementData data, CommonAPIAccess caa, String currElementName, String strExitState) 
	{
		...
		try {
			objHostDetails.startHostReport(currElementName,"RetrieveIntermediarySegmentationInfo", strReqBody, region,(String) data.getSessionData(Constants.S_POLICYINQUIRY_RETRIVEINSURANCEPOLICIESBYPARTY_URL));
			objHostDetails.endHostReport(data,strRespBody , strExitState.equalsIgnoreCase(Constants.ER) ? Constants.ER : Constants.SU, apiRespCode,"");
		} catch (Exception e) {
			data.addToLog(currElementName,"Exception while forming host reporting for RetrieveIntermediarySegmentationInfo call  :: "+e);
			caa.printStackTrace(e);
		}
		return strExitState;
	}
			...
			if(agentsArr!=null) {
				...
				if(agentsArr != null && agentsArr.size() > 0) {
...

					{
						...
						if(lang.equalsIgnoreCase("SP") && prompt.toLowerCase().contains("a specialty dwelling or mobile mome policy")) {
						...
						if(lang.equalsIgnoreCase("SP") && prompt.toLowerCase().contains("specialty dwelling or mobile home")) {
							prompt = prompt.replace("Specialty Dwelling or Mobile Home"," vivienda especial o póliza de casa móvil");
						}
			...
			if(agentsArr != null && agentsArr.size() > 0)
			{
				...
				for(int i = 0; i < agentsArr.size(); i++) 
				{
...

					{
						...
						if(lang.equalsIgnoreCase("SP") && prompt.toLowerCase().contains("specialty dwelling or mobile mome")) {
	...
	{			
		...
		try {
			...
			if(((JSONObject) agentsArr.get(0)).containsKey("policyNumber"))
			{
				String policyNumber = (String)((JSONObject) agentsArr.get(0)).get("policyNumber");
				data.setSessionData("S_API_POLICY_POLICYNUMBER", policyNumber);
				data.addToLog("S_API_POLICY_POLICYNUMBER", policyNumber);

			}
			if(((JSONObject) agentsArr.get(0)).containsKey("billingAccountNumber"))
			{
				String billingAccountNumber = (String)((JSONObject) agentsArr.get(0)).get("billingAccountNumber");
				data.setSessionData("S_API_POLICY_BILLINGACNUMB", billingAccountNumber);
				data.addToLog("S_API_POLICY_BILLINGACNUMB", billingAccountNumber);

			}
			if(((JSONObject) agentsArr.get(0)).containsKey("effectiveDate"))
			{
				String effectiveDate = (String)((JSONObject) agentsArr.get(0)).get("effectiveDate");
				data.setSessionData("S_API_POLICY_EFFECTIVEDATE", effectiveDate);
				data.addToLog("S_API_POLICY_EFFECTIVEDATE", effectiveDate);

			}
			if(((JSONObject) agentsArr.get(0)).containsKey("suffix"))
			{
				String suffix = (String)((JSONObject) agentsArr.get(0)).get("suffix");
				data.setSessionData("S_API_POLICY_SUFFIX", suffix);
				data.addToLog("S_API_POLICY_SUFFIX", suffix);

			}
			if(((JSONObject) agentsArr.get(0)).containsKey("policyState"))
			{
				String policyState = (String)((JSONObject) agentsArr.get(0)).get("policyState");
				data.setSessionData("S_API_POLICY_POLICYSTATE", policyState);
				data.setSessionData(Constants.S_POLICY_STATE_CODE, policyState);
				data.addToLog("S_API_POLICY_POLICYSTATE", policyState);
			...
			if(((JSONObject) zipDetailsArr.get(0)).containsKey("zip"))
			{
				String Zip = (String)((JSONObject) zipDetailsArr.get(0)).get("zip");
				data.setSessionData("S_API_ZIP", Zip);
				data.addToLog("S_API_ZIP", Zip);
			...
			{
				data.setSessionData("S_API_POLICY_SOURCE", "Met360");
				data.addToLog("S_API_POLICYCODE", "ARS");	
			}else
			{
				data.setSessionData("S_API_POLICY_SOURCE", "ARS"); 
				data.addToLog("S_API_POLICYCODE", "ARS");	
			}	
...

			{
				...
				if (productTypeCounts.containsKey(Constants.PRODUCTTYPE_AUTOA)) {
					autoprodtypecount = productTypeCounts.get(Constants.PRODUCTTYPE_AUTO);
				}
				if (productTypeCounts.containsKey(Constants.PRODUCTTYPE_UMBRELLA)) {
					umbrellaprodtypecount = productTypeCounts.get(Constants.PRODUCTTYPE_UMBRELLA);
	...
	private String apiResponseManupulation_RetriveInsurancePoliciesByParty(DecisionElementData data, CommonAPIAccess caa, String currElementName, String strRespBody) {
		...
		try {
			...
			if(autoPoliciesarr != null && autoPoliciesarr.size() > 0) 
			{
				...
				{
					if (lob.equalsIgnoreCase("RV")) {
						prompt = prompt+" Motor Home or Motorcycle";
					}
					else if (lob.equalsIgnoreCase("AUTO")) {
						prompt = prompt+" Auto";
					}
					...
					if(lang.equalsIgnoreCase("SP") && prompt.toLowerCase().contains("auto")) {
						prompt = prompt.replace(" Auto"," póliza de auto");
					}
					if(lang.equalsIgnoreCase("SP") && prompt.toLowerCase().contains("home")) {
						prompt = prompt.replace(" Home"," póliza de casa");
					}
					if(lang.equalsIgnoreCase("SP") && prompt.toLowerCase().contains("motor home or motorcycle")) {
						prompt = prompt.replace(" Motor Home or Motorcycle"," casa rodante o motocicleta");
					}
	...
	private String apiResponseManupulation_PointPolicyInquiry(DecisionElementData data, CommonAPIAccess caa, String currElementName, String strRespBody) {
		...
		try {
			...
			if(resultsObj != null && resultsObj.size() > 0)
			{
				...
				if(((JSONObject) driversarr.get(0)).containsKey("dateofbirth"))
				{
					String birthDate = (String)((JSONObject) driversarr.get(0)).get("dateofbirth");
					data.setSessionData("S_API_DOB", birthDate);
					data.addToLog("S_API_DOB", birthDate);
				}
				if(((JSONObject) address.get(0)).containsKey("zip"))
				{
		...
		try {
			if(data.getSessionData(Constants.S_FWS_POLICYLOOKUP_URL) != null) {
				...
				if(responses != null) 
				{
					if(responses.containsKey(Constants.REQUEST_BODY)) strReqBody = responses.get(Constants.REQUEST_BODY).toString();
					if(responses.containsKey(Constants.RESPONSE_CODE) && (int) responses.get(Constants.RESPONSE_CODE) == 200 && responses.containsKey(Constants.RESPONSE_BODY)) {
						data.addToLog(currElementName, "Set SIDA_HOST_004 : FWSPolicyLookup API Response into session with the key name of "+currElementName+Constants._RESP);
			...
			data.addToLog(currElementName,"Exception in SIDA_HOST_004  FWSPolicyLookup API call  :: "+e);
			caa.printStackTrace(e);
		}
