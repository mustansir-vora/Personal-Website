...
public class CallData implements Serializable{
	...
	private String strContainableCount="";
	private String strContainedCount="";
	private String strPartiallyContainedCount="";
	//Caller verification Venkatesh K M
		private String strCallerVerification="";
	...
	protected String getXML(EndEvent pEvent){
		...
		for(int i=0;i<intNoOfAnnouncement;i++){
			...
			String strAnnouncementStartTime		= 	"";
			String strAnnouncementEndTime		= 	"";
			int sequenceNum = 0;
			...
			try{
				...
				strAnnouncementStartTime = objAnnouncement.getCalAnnouncementStartTime();
				logger.info("strAnnouncementStartTime in Call Data :"+strAnnouncementStartTime);
			
			}catch(Exception e){
				strAnnouncementStartTime = "";
			}
			try {
				strAnnouncementEndTime = objAnnouncement.getCalAnnouncementEndTime();
				logger.info("strAnnouncementEndTime in Call Data :"+strAnnouncementEndTime);
			}catch(Exception e) {
				strAnnouncementEndTime = "";
			}
			...
			if(!strAnnouncementId.equals("")){
				...
				sbAnnouncementsXML.append(strAnnouncementStartTime);
				sbAnnouncementsXML.append("</ANNOUNCEMENT_STARTTIME><ANNOUNCEMENT_ENDTIME>");
				sbAnnouncementsXML.append(strAnnouncementEndTime);
		...
		for(int i=0;i<intNoOfMenus;i++){
			...
			String strNoMatchCount      =   "";
			String strNoInputCount      =   "";
			String combinedCount		=   "";
			String strNluUserUtterance     =   "";
			int sequenceNum = 0;
			
			//START:GDF Connectivity Issue retries changes
			String strHTRetryCount ="";
			...
			//START - CS1240948 :: Fetch NoMatch & NoInput Counts from Bean Object
			try{
				strNoMatchCount = objMenu.getstrNo_Match();
				if(strNoMatchCount==null) {
					strNoMatchCount="0";
				}
			}catch(Exception e){
				strNoMatchCount = "0";
			...
			try{
				strNoInputCount = objMenu.getstrNo_Input();
				if(strNoInputCount==null) {
					strNoInputCount="0";
				}
			}catch(Exception e){
				strNoInputCount = "0";
			...
			//START:GDF Connectivity Issue retries changes
			try {
				strHTRetryCount = objMenu.getStrHoteventRetry();
				if(strHTRetryCount==null) {
					strHTRetryCount="";
				}
			}catch(Exception e) {
				strHTRetryCount="";
			...
			//END:GDF Connectivity Issue retries changes
			combinedCount = String.valueOf(Integer.parseInt(strNoMatchCount) + Integer.parseInt(strNoInputCount));
			//END - CS1240948 :: Fetch NoMatch & NoInput Counts from Bean Object
			//START : All Bu's NLU User Utterances
			try {
				strNluUserUtterance = objMenu.getStrNluMenuUtterance();
				if(strNluUserUtterance==null) {
					strNluUserUtterance="";
				}
			}catch(Exception e) {
				strNluUserUtterance="";
			...
			if(!strMenuId.equals("") && !strMenuStartTime.equals("") ){
				...
				//START - CS1240948 :: Add NoMatch & NoInput Counts in Call Data JSON Packet
				sbMenusXML.append("<NO_MATCH>");
				sbMenusXML.append(strNoMatchCount);
				sbMenusXML.append("</NO_MATCH><NO_INPUT>");
				sbMenusXML.append(strNoInputCount);
				sbMenusXML.append("</NO_INPUT><MENU_RESERVE1>");
				sbMenusXML.append(combinedCount);
				//START:GDF Connectivity Issue retries changes
				sbMenusXML.append("</MENU_RESERVE1><MENU_RESERVE2>");
				sbMenusXML.append(strHTRetryCount);
				sbMenusXML.append("</MENU_RESERVE2><MENU_RESERVE3>");
				//END:GDF Connectivity Issue retries changes
				//END - CS1240948 :: Add NoMatch & NoInput Counts in Call Data JSON Packet
				//START : All Bu's NLU User Utterances
				sbMenusXML.append(strNluUserUtterance);
				sbMenusXML.append("</MENU_RESERVE3><MENU_RESERVE4></MENU_RESERVE4>");
				//END : ALL Bu's NLU User Utterances
				sbMenusXML.append("</MENU_DETAIL>");
				logger.info("sbMenusXML : "+sbMenusXML.toString());
		...
		try {
			...
			for(int i=0; i<intNoOfHostDetails; i++)
			{
				...
				String strHostEndPoint="";
				String strHostRegion="";
				String strHostString="";
				...
				try{
					strHostEndPoint = objHostInteractions.getHostEndpoint();
				}catch(Exception e){
					strHostEndPoint = "";
				}
				try{
					strHostRegion = objHostInteractions.getHostRegion();
				}catch(Exception e){
					strHostRegion = "";
				}
				
				try{
					strHostString = objHostInteractions.getStrHostString();
				}catch(Exception e){
					strHostString = "";
				}
				...
				sbHostXML.append("</HOST_COUNTER><HOST_RESERVE1>");
				sbHostXML.append("<![CDATA["+strHostRegion+"]]>");
				sbHostXML.append("</HOST_RESERVE1><HOST_RESERVE2>");
				sbHostXML.append("<![CDATA[").append(strHostEndPoint).append("]]>");
				sbHostXML.append("</HOST_RESERVE2><HOST_RESERVE3>");
				sbHostXML.append("<![CDATA["+strHostString+"]]>");
				sbHostXML.append("</HOST_RESERVE3><HOST_RESERVE4></HOST_RESERVE4></HOST_DETAIL>");
			}
		...
		sbTmp.append(strICMCallID + "_" + strSessionID);
		sbTmp.append("</CALL_ID>");
		...
		sbTmp.append("</FINAL_INTENT><CONTAINABLE_COUNT>");
		sbTmp.append(strContainableCount);
		sbTmp.append("</CONTAINABLE_COUNT><CONTAINED_COUNT>");
		sbTmp.append(strContainedCount);
		sbTmp.append("</CONTAINED_COUNT><PARTIALLY_CONTAINED_COUNT>");
		sbTmp.append(strPartiallyContainedCount);
		sbTmp.append("</PARTIALLY_CONTAINED_COUNT><ERR_REASON>");
		...
		sbTmp.append("</ERR_REASON><RESERVE1></RESERVE1><RESERVE2></RESERVE2><RESERVE3>");
		//sbTmp.append("CVFlag:");
		sbTmp.append(strCallerVerification);
		sbTmp.append("</RESERVE3><RESERVE4>");
		sbTmp.append(strICMCallID);
		sbTmp.append("</RESERVE4></CALLINFO>");
	...
	public String getstrContainableCount() {
		return strContainableCount;
	}

	public void setstrContainableCount(String strContainableCount) {
		this.strContainableCount = strContainableCount;
	}

	public String getstrContainedCount() {
		return strContainedCount;
	}

	public void setstrContainedCount(String strContainedCount) {
		this.strContainedCount = strContainedCount;
	}

	public String getstrPartiallyContainedCount() {
		return strPartiallyContainedCount;
	}

	public void setstrPartiallyContainedCount(String strPartiallyContainedCount) {
		this.strPartiallyContainedCount = strPartiallyContainedCount;
	}
	
	public String getstrCallerVerification() {
		return strCallerVerification;
	}

	public void setstrCallerVerification(String strCallerVerification) {
		this.strCallerVerification = strCallerVerification;
	}
