...
public class FMOM_MN_005_VALUE extends DecisionElementBase {
			...
			if (authCheck != null && ("TRUE").equalsIgnoreCase(authCheck)) {
				...
				if (productCode != null && !productCode.isEmpty()) {
					...
					if (productCode.equals("077") || productCode.equals("255") || productCode.equals("77")) {
						...
						caa.createMSPKey(caa, data, "FMOM_MN_005", "MOBILE HOME");
						...
						caa.createMSPKey(caa, data, "FMOM_MN_005", "SPECIALITY DWELLING");
					} else if (productCode.equals("276")) {
						strExitState = Constants.MOTORCYCLE;
						strExitStateTemp = Constants.MOTORCYCLE;
						caa.createMSPKey(caa, data, "FMOM_MN_005", "OFF ROAD VEHICLE");
						...
						caa.createMSPKey(caa, data, "FMOM_MN_005", "BOAT");
					}
					...
					if (productType.equalsIgnoreCase("AUTO")) {
						...
						caa.createMSPKey(caa, data, "FMOM_MN_005", "AUTO");	
					}
						...
						strExitState = Constants.BOAT_OR_WATERCRAFT;
						strExitStateTemp = Constants.BOAT_OR_WATERCRAFT;
						caa.createMSPKey(caa, data, "FMOM_MN_005", "BOAT WATERCRAFT");	
					}
						...
						if(subProductType!=null && !subProductType.isEmpty()) {
							if(subProductType.toLowerCase().equalsIgnoreCase("motor home policy") || subProductType.toLowerCase().equalsIgnoreCase("Motor Home") || subProductType.toLowerCase().equalsIgnoreCase("TX Commercial Mobile Home Liability Policy")) {
								strExitState = Constants.MOTOR_HOME;
								strExitStateTemp = Constants.MOTOR_HOME;
								caa.createMSPKey(caa, data, "FMOM_MN_005", "MOTOR HOME TRAVEL");
								...
								strExitState = Constants.MOTORCYCLE;
								strExitStateTemp = Constants.MOTORCYCLE;
								caa.createMSPKey(caa, data, "FMOM_MN_005", "MOTOR CYCLE OFF ROAD");
							}else {
						...
						if(subProductType!=null && !subProductType.isEmpty()) {
							if(subProductType.toLowerCase().equalsIgnoreCase("specialty dwelling") || subProductType.toLowerCase().equalsIgnoreCase("Specialty Dwelling")) {
								strExitState = Constants.SPECIALTY_DWELLING;
								strExitStateTemp = Constants.SPECIALTY_DWELLING;
								caa.createMSPKey(caa, data, "FMOM_MN_005", "SPECIALTY DWELLING");
							}
				...
				String strReturnValue = (String) data.getElementData("FMOM_MN_005_Call", "Return_Value");
				data.addToLog(currElementName, "FMOM_MN_005_Call : Return_Value :: " + strReturnValue);
				data.setSessionData("FMOM_MN_005_VALUE", strReturnValue);
				...
				if (null != strReturnValue) {
					if (strReturnValue.equalsIgnoreCase(Constants.NOINPUT)) {
						...
						strExitState = Constants.MOTORCYCLE;
						strExitStateTemp = Constants.MOTORCYCLE;
						caa.createMSPKey(caa, data, "FMOM_MN_005", "MOTORCYCLE");
					} else if (strReturnValue.equalsIgnoreCase(Constants.MOTOR_HOME)) {
						strExitState = Constants.MOTOR_HOME;
						strExitStateTemp = Constants.MOTOR_HOME;
						caa.createMSPKey(caa, data, "FMOM_MN_005", "MOTOR HOME");
					} else if (strReturnValue.equalsIgnoreCase(Constants.TRAVEL_TRAILER)) {
						strExitState = Constants.MOTOR_HOME;
						strExitStateTemp = Constants.MOTOR_HOME;
						caa.createMSPKey(caa, data, "FMOM_MN_005", "TRAVEL TRAILER");
						...
						caa.createMSPKey(caa, data, "FMOM_MN_005", "REPRESENTATIVE");
					} else {
						strExitState = Constants.ER;
						strExitStateTemp = Constants.ER;
						data.addToLog(currElementName,
								"FMOM_MN_005 if the value from GDF is null :: " + strReturnValue);
					}
				} else {
					strExitState = Constants.ER;
					strExitStateTemp = Constants.ER;
					data.addToLog(currElementName, "FMOM_MN_005 if the value from GDF is null :: " + strReturnValue);
				}

				data.setSessionData("S_FMOM_MN_005_RETURN_VALUE", strExitStateTemp);
				data.addToLog(currElementName, "S_FMOM_MN_005_RETURN_VALUE :: " + strExitStateTemp);
			}
			data.setSessionData("S_FMOM_MN_005_RETURN_VALUE", strExitStateTemp);
			data.addToLog(currElementName, "S_FMOM_MN_005_RETURN_VALUE :: " + strExitStateTemp);		
		} 
		catch (Exception e) {
			data.addToLog(currElementName, "Exception in FMOM_MN_005_VALUE :: " + e);
			caa.printStackTrace(e);
		}
		data.addToLog(currElementName, "FMOM_MN_005_VALUE strExitState:: " + strExitState);

		//		String menuExsitState = strExitState;
		//		if (strExitState.contains(" "))
		//			menuExsitState = menuExsitState.replaceAll(" ", "_");
		//		if (null != (String) data.getSessionData("FMOM_MN_005_" + menuExsitState)
		//				&& !((String) data.getSessionData("FMOM_MN_005_" + menuExsitState)).isEmpty())
		//			menuExsitState = (String) data.getSessionData("FMOM_MN_005_" + menuExsitState);
		//		data.addToLog(currElementName, "Final Value of Menu Exit State for FMOM_MN_005: " + menuExsitState);
		//		data.setSessionData(Constants.S_MENU_SELECTION_KEY,
		//				data.getSessionData(Constants.S_BU) + ":FMOM_MN_005:" + menuExsitState);
		//
		//		data.addToLog(currElementName,
		//				"S_MENU_SELECTION_KEY FMOM_MN_005: " + (String) data.getSessionData("S_MENU_SELCTION_KEY"));
