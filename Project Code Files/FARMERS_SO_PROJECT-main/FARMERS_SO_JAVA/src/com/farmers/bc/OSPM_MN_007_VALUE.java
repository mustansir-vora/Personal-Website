...
public class OSPM_MN_007_VALUE extends DecisionElementBase {
	...
	public String doDecision(String currElementName, DecisionElementData data) throws AudiumException {
		...
		try {
			...
			if (authCheck != null && "TRUE".equalsIgnoreCase(authCheck)) {
				...
				if (productCode != null && !productCode.isEmpty()) {
					...
					if (productCode.equals("077") || productCode.equals("255") || productCode.equals("77")) {
						...
						caa.createMSPKey(caa, data, "OSPM_MN_007", "MOTOR HOME TRAVEL");
						...
						caa.createMSPKey(caa, data, "OSPM_MN_007", "BOAT WATERCRAFT");
					} else {
						strExitState = Constants.ER;
						strExitStateTemp = Constants.ER;
					}

					data.setSessionData("S_OSPM_MN_007_RETURN_VALUE", strExitStateTemp);
					data.addToLog(currElementName, "S_OSPM_MN_007_RETURN_VALUE :: " + strExitStateTemp);
					...
					if (productType.equalsIgnoreCase("AUTO")) {
						...
						caa.createMSPKey(caa, data, "OSPM_MN_007", "AUTO");	
					}
				...
				String strReturnValue = (String) data.getElementData("OSPM_MN_007_Call", "Return_Value");
				data.addToLog(currElementName, "OSPM_MN_007_Call : Return_Value :: " + strReturnValue);
				data.setSessionData("OSPM_MN_007_VALUE", strReturnValue);
				if (null != strReturnValue) {
					if (strReturnValue.equalsIgnoreCase(Constants.NOINPUT)) {
						...
						caa.createMSPKey(caa, data, "OSPM_MN_007", "REPRESENTATIVE");
						...
						strExitState = Constants.ER;
						strExitStateTemp = Constants.ER;
						data.addToLog(currElementName,"OSPM_MN_007 if the value from GDF intent is ER :: " + strReturnValue);
					}
				} else {
					strExitState = Constants.ER;
					strExitStateTemp = Constants.ER;
					data.addToLog(currElementName, "OSPM_MN_007 if the value from GDF is null :: " + strReturnValue);
				}
			...
			data.addToLog(currElementName, "Exception in OSPM_MN_007_VALUE :: " + e);
			caa.printStackTrace(e);
		}
		data.addToLog(currElementName, "OSPM_MN_007_VALUE :: " + strExitState);

		//		String menuExsitState = strExitState;
		//		if (strExitState.contains(" "))
		//			menuExsitState = menuExsitState.replaceAll(" ", "_");
		//		if (null != (String) data.getSessionData("OSPM_MN_007_" + menuExsitState)
		//				&& !((String) data.getSessionData("OSPM_MN_007_" + menuExsitState)).isEmpty())
		//			menuExsitState = (String) data.getSessionData("OSPM_MN_007_" + menuExsitState);
		//		data.addToLog(currElementName, "Final Value of Menu Exit State for OSPM_MN_007: " + menuExsitState);
		//		data.setSessionData(Constants.S_MENU_SELECTION_KEY,
		//				data.getSessionData(Constants.S_BU) + ":OSPM_MN_007:" + menuExsitState);
		//		data.addToLog(currElementName,
		//				"S_MENU_SELECTION_KEY OSPM_MN_007: " + (String) data.getSessionData("S_MENU_SELCTION_KEY"));
		return strExitState;
	}
}